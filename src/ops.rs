    ...
    pub fn to_shapes(&self) -> Vec<Vec<PipeShape>> {
        ...
        for scan in scans.iter() {
            ...
            match scan {
                ...
                _ => {
                    panic!("Non-scan in pipe shapes: {:?}", scan)
                }
            }
...
impl fmt::Debug for OutputingIter {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            &OutputingIter::Empty => {
                write!(f, "OutputingIter::Empty")
            }
            &OutputingIter::Single(reg, ..) => {
                write!(f, "OutputingIter::Single({:?})", reg)
            }
            &OutputingIter::Multi(ref regs, ..) => {
                write!(f, "OutputingIter::Multi({:?})", regs)
            }
        }
    }
}
...
impl Internable {
    ...
    pub fn to_string(intern: &Internable) -> String {
        match intern {
            ...
            _ => {
                panic!("to_string on non-string/number")
            }
        }
    }
...
pub fn eve_type_of(params: Vec<&Internable>) -> Option<Internable> {
    match params.get(0) {
        ...
        _ => {
            panic!("Type of called without a valid parameter")
        }
    }
}
